...
object ScalaMapCheckerTest {
  def main(args: Array[String]): Unit = {
    ...
    println(MapChecker.tryMove(new game.Map("map/1.map"),18,19,1))
    println(MapChecker.tryMove(new game.Map("map/1.map"),19,20,3))
    println(MapChecker.tryMove(new game.Map("map/1.map"),23,24,2))
    println(MapChecker.tryMove(new game.Map("map/1.map"),24,7,4))
